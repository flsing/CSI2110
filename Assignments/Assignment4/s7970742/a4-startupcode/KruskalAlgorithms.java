...
public class KruskalAlgorithms<V,E extends Comparable<E>> {
	...
	public LinkedList <Edge<E>>  kruskalClusters (Graph<V,E> g, int k, Partition<Vertex<V>> clusters) {
		...
		while(forest.size()<(g.numVertices()-k)){
			//instantiate
			Edge<E> min =pq.remove();
			Vertex<V> [] vert = g.endVertices(min);
			Vertex<V> a = vert[0];
			Vertex<V> b = vert[1];
			
			//main loop
			if(clusters.find(hash.get(a)) != null && clusters.find(hash.get(b))!= null){
				if (clusters.find(hash.get(a)) != clusters.find(hash.get(b))){
				
					forest.add(min);
					clusters.union(hash.get(a), hash.get(b));
