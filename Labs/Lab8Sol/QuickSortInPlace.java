...
// Changed quick sort method to use random element as pivot
// ==========================================================================
/**
...
public class QuickSortInPlace<T extends Comparable> {
  ...
  protected void quickSort( int _left, int _right )
  {
    ...
    // Pivot selection is randomized now.
    int _size = _right - _left + 1;
    int pivotindex = _left + (int)(Math.random()*_size);     // choose random index in the interval [_left,_right]
    T pivot = d_seq[pivotindex];
    d_seq[pivotindex] = d_seq[_right];         // Swap the pivot element with rightmost element.
    d_seq[_right] = pivot;                     // (Note that the partition funcion expects the pivot to be at _right) 
