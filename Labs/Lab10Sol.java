// $Id: WeightGraph.java,v 1.1 2006/11/18 01:20:40 jlang Exp $
...
// Revision 1.1  2006/11/18 01:20:40  jlang
// Added solution to lab10
//
...
public class WeightGraph {
  ...
  void printAllShortestDistances( String vert  ) {
    ...
    Dijkstra<String,String> dj = new Dijkstra<String,String>();
    dj.execute(sGraph, vSource, WEIGHT);
    // Print shortest path to named cities
    for( Vertex<String> vGoal : sGraph.vertices() ) {
      if ( vGoal.element().length() > 2 ) {
	System.out.println( vSource.element() + " to " +
			    vGoal.element() + " = " +  	
			    dj.getDist( vGoal ));
      }
    }
